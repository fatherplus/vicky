...
**架构全貌**走 `PUT /api/arch/...`（骨架 + 模块正文）。模块正文的叙事骨架见下（每个技术决策仍必答三问：解决什么？为什么是它？不这么做呢？）。

### 节点叙事骨架（引导，不门禁）

模块节点正文（body_md）建议 5 节，路由节点 3 节。缺章节不拒收，但渲染会更好看。
...
> 判定条件表必须与骨架出边的 `condition` 一一对应。

### tech-solution 报告叙事骨架（参考 superpowers design）

技术方案报告按 5 节写，每节结论先行（只读结论句就能拿到论证骨架）：
